...
def check_torch_tensor(x, name, shape=None, dtype=None):
    import torch
    if not (yield isinstance(x, torch.Tensor), f'{name} must be a PyTorch tensor (got {type(x)})'):
        return False
    if shape is not None:
        if not (yield x.shape == shape, f'{name} must be a PyTorch tensor with shape {tuple(shape)} (got {tuple(x.shape)})'):
            return False
    if dtype is not None:
        if not (yield x.dtype == dtype, f'{name} must be a PyTorch tensor of type {dtype} (got {x.dtype})'):
            return False
    return True
